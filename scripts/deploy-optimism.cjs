...
async function main() {
  console.log("Starting deployment on Optimism Sepolia...");
  console.log("Network:", hre.network.name);

  // Optimism Sepolia-specific variables
  const routerAddress = process.env.OPTIMISM_ROUTER_ADDRESS;
  const functionsRouterAddress = process.env.OPTIMISM_FUNCTIONS_ROUTER_ADDRESS;
  const donIdString = process.env.OPTIMISM_DON_ID;
  const subscriptionId = process.env.OPTIMISM_SUBSCRIPTION_ID;
  const mainRouterChainSelector = process.env.OPTIMISM_MAIN_ROUTER_CHAIN_SELECTOR ? BigInt(process.env.OPTIMISM_MAIN_ROUTER_CHAIN_SELECTOR) : undefined;
  const initialMinterAddress = process.env.OPTIMISM_INITIAL_MINTER_ADDRESS || "0x0000000000000000000000000000000000000000";
  ...
  if (!routerAddress || !functionsRouterAddress || !donIdString || subscriptionId === undefined) {
    throw new Error('OPTIMISM_ROUTER_ADDRESS, OPTIMISM_FUNCTIONS_ROUTER_ADDRESS, OPTIMISM_DON_ID, and OPTIMISM_SUBSCRIPTION_ID must be set in .env for MainRouter deployment');
  }
  ...
  if (!routerAddress || mainRouterChainSelector === undefined || !mainRouterAddress) {
    throw new Error('OPTIMISM_ROUTER_ADDRESS, OPTIMISM_MAIN_ROUTER_CHAIN_SELECTOR, and MainRouter address must be available for Minter deployment');
  }
