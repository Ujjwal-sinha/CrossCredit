...
async function main() {
  console.log("Starting deployment on Arbitrum Sepolia...");
  console.log("Network:", hre.network.name);

  // Arbitrum Sepolia-specific variables
  const routerAddress = process.env.ARBITRUM_ROUTER_ADDRESS;
  const functionsRouterAddress = process.env.ARBITRUM_FUNCTIONS_ROUTER_ADDRESS;
  const donIdString = process.env.ARBITRUM_DON_ID;
  const subscriptionId = process.env.ARBITRUM_SUBSCRIPTION_ID;
  const mainRouterChainSelector = process.env.ARBITRUM_MAIN_ROUTER_CHAIN_SELECTOR ? BigInt(process.env.ARBITRUM_MAIN_ROUTER_CHAIN_SELECTOR) : undefined;
  const initialMinterAddress = process.env.ARBITRUM_INITIAL_MINTER_ADDRESS || "0x0000000000000000000000000000000000000000";
  ...
  if (!routerAddress || !functionsRouterAddress || !donIdString || subscriptionId === undefined) {
    throw new Error('ARBITRUM_ROUTER_ADDRESS, ARBITRUM_FUNCTIONS_ROUTER_ADDRESS, ARBITRUM_DON_ID, and ARBITRUM_SUBSCRIPTION_ID must be set in .env for MainRouter deployment');
  }
  ...
  if (!routerAddress || mainRouterChainSelector === undefined || !mainRouterAddress) {
    throw new Error('ARBITRUM_ROUTER_ADDRESS, ARBITRUM_MAIN_ROUTER_CHAIN_SELECTOR, and MainRouter address must be available for Minter deployment');
  }
